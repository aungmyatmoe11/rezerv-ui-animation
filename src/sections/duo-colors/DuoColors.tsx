...
import { DUO_FINISHES, type DuoFinish } from '@/data/duo';
import styles from './DuoColors.module.scss';

const ALT = 'Concept render of a foldable iPhone turning to show its rear finish.';

/**
 * Section 17 — Duo finishes. Scroll turns the device; the picker sits beneath.
 *
 ...
 * Duo act instead of being the one clip that plays to its own clock.
 *
 ...
 *
 * There are no device renders for the Duo, so the selection retints the panel
 * rather than swapping an image it does not have.
 */
export function DuoColors() {
  const [active, setActive] = useState<DuoFinish>(DUO_FINISHES[0]!);
  ...
  return (
    <>
      {/* The headline lives ABOVE the film, not over it.
          ...
          clip in the Duo act — a silver body fills the upper left. A dark band
        ...
        >
          ...
          <div className={styles.controls}>
            <fieldset className={styles.swatches}>
              <legend className="visually-hidden">Choose an Duo finish</legend>
              {DUO_FINISHES.map((finish) => (
                <label
                  key={finish.id}
